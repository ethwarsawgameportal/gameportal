...
const GameCard: React.FC<GameCardProps> = ({ item }) => {
  return (
    <div
      key={item.id}
      className={twMerge(
        "overflow-hidden cursor-not-allowed rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm transition-shadow",
        !item.isComingSoon && "hover:shadow-md cursor-pointer",
      )}
    >
      <div
